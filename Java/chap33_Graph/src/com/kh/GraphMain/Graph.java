...
public class Graph {
	...
	public void addEdge(int source, int des) {
		...
		//source에 해당하는 LinkedList에 des추가
	...
	public void DFS(int startNode) {
		...
		DFUtil(startNode, visited);
	}
	//실제 깊이 우선 탐색 수행
	private void DFUtil(int currentNode, boolean[] visited) {
		//현재 노드를 방문한 것으로 표시
		visited[currentNode] = true;
		System.out.println(currentNode + " ");
		//현재 노드와 연결된 인접 노드로 확인하고, 방문하지 않는 인접 노드를 재귀적(원래의 자리로 돌아오거나 되돌아오는)으로 방문
		for (int neighbor : adjList[currentNode]) {
			if(!visited[neighbor]) {
				DFUtil(neighbor, visited);
			}
		}
	}
